...
def main_relat_despesas(list, path):
...
# Letter size paper, use inches as unit of measure
    ...
    data = [['Num. Título','Descrição','Fornecedor','Valor', 'Obra', 'Data da Baixa']]
    ...
    for x in list:
        ...
        for c in x:
            if(c == date):
                c   = datetime.strptime(str(date)[0:20], "%Y-%m-%d %H:%M:%S").strftime("%d-%m-%Y")
            temp_array.append(c)
